 ...
 */
export interface NodejsEdgeFunctionProps extends Partial<cloudfront.experimental.EdgeFunctionProps> {
  /**
...
function findLockFile(depsLockFilePath?: string): string {
  ...
  const lockFile =
    findUp(PackageManager.PNPM.lockFile) ?? findUp(PackageManager.YARN.lockFile) ?? findUp(PackageManager.NPM.lockFile);
